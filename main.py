...
def prepareText(para,stopWordFlag):
    ...
    textForChaining = preCoRefPreparation(para)
    para = splitTextSen(textForChaining)
...
def taggingText(text):
    # Commented out code is used if you are using a tagger you have trained (A none default tagger)
    # t0 = NgramTagger()
...
def addCoRefrenceRels(chainText,objText):
    chains = chaining(chainText) 
    for i in range(len(chains)):
        for j in range(len(chains[i])):
            obj = findToken(chains[i][j],objText)
            obj.setCoRefNum(i)
